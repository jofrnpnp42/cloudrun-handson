...
Google Cloud ConsoleからArtifact Registryのページに移動して機能をONにするだけでプロジェクト全体での脆弱性スキャンが有効化されま/す。

 Artifact Registry に移動

設定 に移動します。

`[スキャン:オフ]`となっていることを確認し、 有効にする  をクリックします。
...
ルール一覧：https://cloud.google.com/armor/docs/waf-rules
...
参考：https://cloud.google.com/armor/docs/configure-security-policies?hl=ja#attach-policies
...
設定がバックエンドサービスに反映されるまで、少し時間がかかります。HTTP200が応答される場合、しばらく時間をおきましょう。
...
Cloud Deploy に移動
 デリバリーパイプライン  → `[cnsrun-frontend]`を選択し、
デプロイが保留されていることが確認できます。
3. 保留中の絵の下にある確認ボタンを押します。
4. 行の一番右にあるREVIEWボタンを押します。
5. 承認画面から、どういった変更が発生したかを確認できます。内容を確認したら、承認ボタンを押してください。
...
2. SLO タブを選択します。
3. SLOがまだ作られていないことを確認し、SLOを作成ボタンを押します。

### **SLIの設定**

まずSLIを決定します。`[指標の選択]`では`[可用性]`を選択し、`[リクエストベース]`のSLIとして続行を押します。

SLI の詳細では、そのまま続行を押します。
...
最後の確認画面でSLOの設定内容を確認し、SLOを作成を押下して完了です。
...
echo $KEY_VERSION
```
6. CLOUD KMSからインポートを選択します。
7. メモしておいたリソースIDを入力します。
7. 詳細設定のアコーディオンを開いて、Container Analysis メモを自動生成するにチェックを付けます。
8. 作成ボタンを押します。
...
通常はCI/CDプロセスの中で証明書を作成しますが、シンプルに試すために手動でも作成します。
...
1. ポリシータブを選択します。
2. ポリシーを編集を編集を押します。
3. デフォルトのルールとして、証明書を要求にチェックを入れます。
4. 要求した証明書を認証する認証者（attestor)を指定するために、 認証者の追加を押します。
5. `プロジェクトと認証者の名前により追加`にチェックを入れたままにし、アテスターの`[認証者の名前]`として、`cnsrun-attestor`を入力して、`[認証者の追加]`ボタンを押します。
6. ポリシーを保存を押して編集を完了します。
...
1. フロントエンドアプリケーション `cnsrun-frontend` を選択します。
2. 新しいリビジョンの編集とデプロイを押します。
3. 証明書がないコンテナイメージに切り替えます。`[コンテナイメージのURL]`から 選択 を押して、古いハッシュのイメージを選択します。
4. このリビジョンをすぐに利用するにチェックを入れます。
5. デプロイ を押します。
...
<strong>注意：Cloud Domainでドメインを取得する場合、ドメインの取得費用がかかります。すでに独自ドメインを取得している場合や無料ドメインを利用する場合はCloud Domainの手順はスキップをしてください。</strong>
...
再掲：Cloud Domainでドメインを取得する場合、ドメインの取得費用がかかります。すでに独自ドメインを取得している場合や無料ドメインを利用する場合はCloud Domainの手順はスキップをしてください。
...
DNSレコードの伝搬に時間がかかる：https://cloud.google.com/load-balancing/docs/ssl-certificates/google-managed-certs?hl=ja#dns_record_propagation_time
...
https://cloud.google.com/load-balancing/docs/ssl-certificates/google-managed-certs?hl=ja#step_5_test_with_openssl
